...
with open("./Org1_privateKey.txt", "rb") as pb:
...
while True:
    while not auth_status:
        auth_response = requests.post(url_auth, json={'org_id':'org1'})
    ...
    if auth_status:
        ...
        if chk==1:
            ...
            val2 = -val1
            data_to_send = f'{{"org1": {val1}, "org2": {val2}, "ss_key": "{session_key}", "sender":"Org1"}}'
        else:
            val1 = random.randint(-78,80)
            data_to_send = f'{{"org1": {val1}, "org2": 0, "ss_key": "{session_key}", "sender":"Org1"}}'
