...
async function main() {
    try {
        ...
        await (0, CAUtil_1.enrollAdmin)(caClient, wallet, mspOrg1);
        await (0, CAUtil_1.registerAndEnrollUser)(caClient, wallet, mspOrg1, org1UserId, 'org1.department1');
        ...
        try {
            ...
            function getStateRecord() {
                return new Promise((resolve, reject) => {
                    db.get("SELECT value FROM data WHERE key = 'stateData'", (err, row) => {
                        if (err) {
                            reject(err);
                        }
                        else {
            ...
            function custom_Encrypt(msg, Pub_key) {
            ...
            async function initializeData(key) {
                try {
            ...
            app.post('/authenticate', express_1.default.json(), async (req, res) => {
                try {
            ...
            function custom_Decrypt(emsg, Priv_key) {
            ...
            async function processRequest(dataFromPython, callback) {
